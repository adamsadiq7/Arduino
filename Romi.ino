...
void right_motor(float r_speed){
  if (r_speed < 0){
...
void bangBang(){
  ...
  float middle = middle_sensor.readCalibrated();
  float left = left_sensor.readCalibrated();
  float right = right_sensor.readCalibrated();
...
void rotateUntil(){
  ...
  if (theta > goal && !wrongSide && !checkedSide){
    wrongSide = true;
    checkedSide = true;
  }
  else{
